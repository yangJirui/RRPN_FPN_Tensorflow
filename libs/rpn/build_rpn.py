...
class RPN(object):
    def __init__(self, net_name, inputs, gtboxes_and_label,
        ...
        self.gtboxes_and_label = gtboxes_and_label  # shape is [M. 6],
        ...
        self.scope = scope
        self.num_of_anchors_per_location = len(anchor_scales) * len(anchor_ratios) * len(anchor_angles)
        ...
        elif cfgs.FEATURE_PYRAMID_MODE == 1:
            self.feature_pyramid = self.build_dense_feature_pyramid()
        elif cfgs.FEATURE_PYRAMID_MODE == 2:
            self.feature_pyramid = self.build_u_feature_fusion_network()
        else:
            raise ValueError("build feature pyramid mode erro--yjr")
    ...
    def make_rotate_anchors(self):
        with tf.variable_scope('make_rotate_anchors'):
            ...
            with tf.name_scope('make_rotate_anchors_all_level'):
                for level, base_anchor_size, anchor_stride \
                        in zip(level_list, self.base_anchor_size_list, self.anchor_stride[0]):
                    '''
                    original paper:
                    ...
                    featuremap_height, featuremap_width = tf.cast(featuremap_height, tf.float32), \
                                                          tf.cast(featuremap_width, tf.float32)
    ...
    def rpn_find_positive_negative_samples(self, anchors):
        ...
        with tf.variable_scope('rpn_find_positive_negative_samples'):
            ...
            # [N, M]
            ious = tf_wrapper.get_iou_matrix_tf(anchors, gtboxes, use_gpu=cfgs.IOU_USE_GPU, gpu_id=0)
